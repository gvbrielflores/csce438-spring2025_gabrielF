...
public:
  Client(const std::string& hname,
	 ...
	 const std::string& p, 
   const std::string& cP)
    :hostname(hname), username(uname), port(p), coordPort(cP) {}
...
private:
  ...
  // Stub to interact with coordinator
...
///////////////////////////////////////////////////////////
    // Create stub to connect to coordinator
    if (coordPort != "-1") {
      std::string coordinatorConnection = hostname + ":" + coordPort;
      auto coordinatorChannel = grpc::CreateChannel(coordinatorConnection, grpc::InsecureChannelCredentials());
      this->coordStub_ = CoordService::NewStub(coordinatorChannel);

      // Get info of assigned server
...
int main(int argc, char** argv) {
  ...
  Client myc(hostname, username, port, coordPort);
